...
현재 소비 패턴을 기반으로 남은 기간 생존비와 미래 사용 가능 자산을 예측합니다.
- **🪖 교관의 평가**: **OpenAI GPT-4o-mini**가 당신의 소비 데이터를 분석하여 뼈 때리는 조언과 즉시 실행 명령을 하달합니다.
- **📆 일일 생존비**: 남은 일수와 예산을 기반으로 하루에 사용할 수 있는 금액을 계산하고, 예산 초과 여부를 즉시 경고합니다.
- **🔮 희망회로**: 평균 월 지출과 예산을 기반으로 예상 저축액을 계산하고, 그 금액에 따라 여행지(또는 훈련소) 목표를 지도에 시각화하여 동기부여를 제공합니다.
