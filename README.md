...
---
## 🚀 향후 개선 방안 (Future Roadmap)

텅장 훈련소는 더 강력한 자산 관리 시스템으로 거듭나기 위해 다음과 같은 업데이트를 계획하고 있습니다.

### 1. 🔐 훈련병 신원 확인 시스템 (User Authentication)
현재는 누구나 접근 가능한 오픈된 훈련소입니다. 개별 훈련병의 프라이버시를 보호하고 개인화된 훈련을 제공하기 위해 **회원가입 및 로그인 기능**을 도입할 예정입니다.
* **계획:** `Streamlit-Authenticator` 라이브러리 또는 OAuth 2.0을 활용하여 보안이 강화된 로그인 시스템 구축
* **기대 효과:** 나만의 훈련 기록 보호 및 다중 사용자 접속 환경 지원

### 2. 🗄️ 개인별 복무 기록 관리 (User-Specific DB)
로그인 기능과 연동하여, 훈련병마다 독립된 데이터베이스 공간을 제공합니다.
* **계획:** MySQL 데이터베이스 스키마 고도화 (Users 테이블 추가 및 `user_id`를 외래키로 활용하여 지출 내역 매핑)
* **기대 효과:** A 훈련병의 낭비 내역이 B 훈련병에게 노출되지 않는 **완벽한 데이터 격리** 구현

### 3. 💰 보급품 및 월급 관리 (Income Management)
현재는 '지출(전투력 손실)' 방어에 집중되어 있습니다. 진정한 자산 증식을 위해 수입(보급) 관리 기능을 추가합니다.
* **계획:** 수입 내역(월급, 용돈, 보너스 등) 입력 폼 및 수입 대비 지출 비율(저축률) 분석 대시보드 추가
* **기대 효과:** 단순 지출 방어를 넘어, **순자산(Net Worth) 증가**를 목표로 하는 종합 자산 관리 솔루션으로 진화
