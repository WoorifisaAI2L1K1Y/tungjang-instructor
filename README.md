...
| 아이콘 | 유형 | 정의 | 예시 | 판정 |
| :---: | :--- | :--- | :--- | :---: |
| 🔴 | **게으름 (Laziness)** | 귀찮음과 편리함에 굴복하여 발생한 비용 | 배달음식, 택시, 연체료 | **낭비** |
| 🟠 | **충동 (Impulse)** | 계획 없이 순간의 감정에 휘둘린 지출 | 카페, 홧김비용, 유흥, 술 | **낭비** |
...
훈련의 기본은 철저한 기록입니다. 모든 지출은 정해진 **분류 체계**에 따라 보고됩니다.

* **📝 작전 기록**: 날짜, 시간, 금액, 메모를 입력하여 DB에 즉시 보고합니다.
* **📅 월간 캘린더**: `Streamlit Calendar`를 활용해 날짜별 지출 전황을 한눈에 파악하고, 클릭 시 상세 내역을 브리핑합니다.

#### **[ 🗂 자금 분류 체계 (Category System) ]**
입력된 소비 내역은 아래의 카테고리로 분류된 후, **4가지 페르소나**로 매핑되어 분석됩니다.
...
- **🔴 낭비 (Waste):** `충동` (순간의 감정), `게으름` (귀찮음 비용)
- **🔵 호흡 (Breathing):** 생존을 위한 필수 고정비
- **🟢 성장 (Growth):** 미래를 위한 투자

##### 2. 📊 월별 리포트 (Monthly Report - Tab 1)
- **KPI 대시보드:** 총 지출, 낭비 비용(비중), 고정비, 투자 비용을 한눈에 확인합니다.
- **데이터 시각화:**
  - **파이 차트:** 지출 성격(충동, 게으름, 호흡, 성장) 비중 분석
  - **바 차트:** 지출 상위 5개 카테고리 분석
- **상세 내역:** 필터링된 월별 상세 지출 내역 테이블을 제공합니다.

##### 3. 🔥 행동 패턴 분석 (Pattern Analysis - Tab 2)
- **상관관계 분석 (Scatter Plot):** '낭비'가 '총 지출'에 미치는 영향을 분석합니다. 상관계수에 따라 교관의 상태(화남, 걱정, 온화 등)가 변합니다.
- **요일/시간별 히트맵 (Heatmap):** 일주일 중 언제, 몇 시에 지출이 집중되는지 시각화하여 취약 시간대를 파악합니다.
- **교관의 피드백:** 데이터 분석 결과에 따라 동적으로 변하는 교관의 멘트와 이미지를 출력합니다.
