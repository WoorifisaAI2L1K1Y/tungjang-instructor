...
def main():
    ...
    with tab1:
        ...
        # [좌측] 소비 성격 비중 (파이차트)
        with col_pie:
            st.subheader(f"🎨 {selected_month} 소비 성격 비중")
        ...
        # [우측] 카테고리 비중 (바차트)
        with col_bar:
            st.subheader(f"💸 {selected_month} 소비 상위 Top 5")
...
# --- TAB 2: 패턴 분석 ---
    with tab2:
        # [수정됨] 소비 유형 정의 표 (st.columns로 가운데 정렬)
        ...
        with col_table:
            ...
            | **충동** | 계획 없이 순간의 감정에 휘둘린 소비 | 카페, 홧김비용, 유흥, 술 | 🔴**낭비** |
        ...
        with col_upper_left:
            st.markdown("""
            ##### :red[낭비는 너가 게으르고, 충동 구매를 한 소비이다.]
            ##### 📉 아래는 너의 낭비가 총 소비에 미치는 영향이다!
            """)
            ...
            if len(monthly_agg) > 1:
                if corr_value >= 0.7:
                    ...
                    bg_color = "#ffeaea" 
                    status_text = f"낭비가 총 소비을 <span style='color: #e03131;'>직접적으로 폭발시키는</span> 상관계수가 {val_html}이다!<br>정신이 있는 건가?! 당장 충동을 억제하고 실시! 😡"
                elif corr_value >= 0.3:
                    img_path = os.path.join(img_dir, '3-짜증.png')
                    bg_color = "#fff3cd"
                    status_text = f"낭비가 늘면 소비도 <span style='color: #e8590c;'>따라서 증가하는</span> 상관계수가 {val_html}다!<br>경고한다! 너의 낭비가 심각하다. 정신 차려라! 😠"
                elif corr_value > -0.3:
                    img_path = os.path.join(img_dir, '1-온화.png')
                    bg_color = "#d4edda"
                    status_text = f"낭비와 소비가 <span style='color: #2b8a3e;'>서로 영향이 없는</span> 상관계수가 {val_html}.<br>보고! 특이사항 없음. 생명 유지비(고정비)를 점검하라. 🤔"
                else:
                    img_path = os.path.join(img_dir, '2-걱정.png')
                    bg_color = "#e2e3e5"
                    status_text = f"낭비를 줄였는데 소비가 늘어나는 <span style='color: #5f3dc4;'>역방향</span> 상관계수가 {val_html} 감지!<br>비상! 기현상이다. 정밀 타격이 필요하다! 😨"
        ...
        with col_upper_right:
            st.markdown("##### 🔥 언제 소비가 가장 많은지 보여주겠다.")
            
            # 가이드 말풍선 로직
            img_path_guide = os.path.join(img_dir, '5-교관의_한마디.png')
            bubble_bg_color = "#e7f5ff"
            guide_text = "💡 <span style='color: #0b7285; font-weight: 600;'>히트맵 판별법</span>: 가로축은 <span style='color: #1c7ed6;'>시간</span>, 세로축은 <span style='color: #1c7ed6;'>요일</span>이다.<br>색이 <span style='color: #e03131;'>붉을수록</span> 해당 시간대에 소비가 극심하다는 뜻이다!"
        ...
        with col_chart_right:
            title_text = f"선택된 유형({', '.join(selected_types)})의 전체 소비 히트맵" if selected_types else "유형을 선택하라"
            ...
            if not target_df.empty:
                ...
                st.markdown(f"**🏆 선택 항목 합산 소비 Top 3**")
                ...
                st.warning(f"선택한 유형에 해당하는 소비 내역이 없다.")
